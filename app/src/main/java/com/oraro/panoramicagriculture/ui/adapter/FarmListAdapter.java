 ...
 * @author
 */
public class FarmListAdapter extends BaseListAdapter<Object> implements BaseListAdapter.OnItemClickListener {
    private MapHelperManager mapHelperManager;
    private Context context;
    private  PurchaseDCActivity purchaseDCActivity;
    public FarmListAdapter(Context context, int mode, PurchaseDCActivity activity) {
        super(context, mode);
        this.context = context;
        purchaseDCActivity=activity;
        mapHelperManager = MapHelperManager.getInstance();
        this.setOnItemClickListener(this);
    }

    public FarmListAdapter(Context context, List<Object> items, int mode) {
    ...
    public void onItemClick(int position, long id, View view) {
        ...
        if (item instanceof FarmEntity) {
            FarmEntity farmEntity = (FarmEntity) item;
            intent.putExtra("pointdcId", farmEntity.getFarmId());
            intent.putExtra("pointname",farmEntity.getFarmName());
            LogUtils.e("jw","pointdcId....="+farmEntity.getFarmId()+"pointname...name="+farmEntity.getFarmName());
        }
        purchaseDCActivity.setResult(Activity.RESULT_OK,intent);
        purchaseDCActivity.finish();
