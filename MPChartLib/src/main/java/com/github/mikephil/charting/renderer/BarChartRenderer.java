...
public class BarChartRenderer extends BarLineScatterCandleBubbleRenderer {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        final boolean isGaintColor = dataSet.getGradientColor().length == 2;
        if (isSingleColor) {
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            if (isGaintColor) {
                mRenderPaint.setShader(new LinearGradient(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                        buffer.buffer[j + 3], dataSet.getGradientColor()[0], dataSet.getGradientColor()[1], Shader.TileMode.MIRROR));
            }
