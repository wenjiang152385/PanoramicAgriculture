...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    protected void drawCubicBezier(ILineDataSet dataSet) {
        ...
        float phaseY = mAnimator.getPhaseY();
        float[] startPath = new float[2];
        float[] endPath = new float[2];
        ...
        if (mXBounds.range >= 1) {
            ...
            //wjq
            cubicPath.moveTo(cur.getX(), cur.getY() * phaseY);
            startPath = new float[]{cur.getX(), cur.getY() * phaseY};
            for (int j = mXBounds.min + 1; j <= mXBounds.range + mXBounds.min; j++) {
                ...
                cubicPath.cubicTo(prev.getX() + prevDx, (prev.getY() + prevDy) * phaseY,
                    ...
                    endPath = new float[]{cur.getX(), cur.getY() * phaseY};
...
//        mRenderPaint.setColor(dataSet.getColor());
        ...
        trans.pathValueToPixel(cubicPath);
        //wjq
        trans.pointValuesToPixel(startPath);
        trans.pointValuesToPixel(endPath);
        Paint cubicBezierPaint = new Paint();
        cubicBezierPaint.setAntiAlias(true);

        if (dataSet.getType() == 1) {
            cubicBezierPaint.setShader(new LinearGradient(
                    startPath[0],
                    startPath[1],
                    endPath[0],
                    endPath[1],
                    //开始颜色
                    Color.parseColor("#1dcdb3"),
                    //结束颜色
                    Color.parseColor("#257cc4"),
                    Shader.TileMode.MIRROR));
        }else if ((dataSet.getType() == 0) ){
            cubicBezierPaint.setShader(new LinearGradient(
                    startPath[0],
                    startPath[1],
                    endPath[0],
                    endPath[1],
                    Color.parseColor("#fadd00"),
                    Color.parseColor("#ff8300"),
                    Shader.TileMode.MIRROR));
        }else if ((dataSet.getType() == -1)) {
            cubicBezierPaint.setColor(Color.BLACK);
        }

        cubicBezierPaint
                .setStyle(Paint.Style.STROKE);
        cubicBezierPaint.setStrokeWidth(dataSet.getLineWidth());
        mBitmapCanvas.drawPath(cubicPath, cubicBezierPaint);
        mRenderPaint.setPathEffect(null);
    }
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            if (e1 != null) {
                ...
                for (int x = mXBounds.min; x <= mXBounds.range + mXBounds.min; x++) {
                    ...
                    if (x == 0) {
                        startPath[0] = e1.getX();
                        startPath[1] = e1.getY() * phaseY;
                    }
                    if (x == mXBounds.range + mXBounds.min) {
                        endPath[0] = e2.getX();
                        endPath[1] = e2.getY() * phaseY;
                    }
                ...
                if (j > 0) {
                    ...
                    trans.pointValuesToPixel(startPath);
                    trans.pointValuesToPixel(endPath);
                    Paint shaderPaint = mRenderPaint;
                    final int size = Math.max((mXBounds.range + 1) * pointsPerEntryPair, pointsPerEntryPair) * 2;

                    if (dataSet.getType() == 1) {
                        shaderPaint.setShader(new LinearGradient(
                                startPath[0],
                                startPath[1],
                                endPath[0],
                                endPath[1],
                    ...
                    if (dataSet.getType() == -1) {
                        ...
                        canvas.drawLines(mLineBuffer, 0, size, shaderPaint);
                    }
                }
            }
        }
    ...
    public Map<Integer,List<Highlight>> getPointLists() {
        return mMap;
    }
    ...
    private Map<Integer,List<Highlight>> mMap;
    protected void drawCircles(Canvas c) {
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            for (int j = mXBounds.min; j <= boundsRangeCount; j++) {
                ...
                Highlight highlight = new Highlight(mCirclesBuffer[0],mCirclesBuffer[1],i);
                mMap.get(i).add(highlight);
