...
public class YAxisRenderer extends AxisRenderer {
    ...
    public void renderGridLines(Canvas c) {
        ...
        if (mYAxis.isDrawGridLinesEnabled()) {
            ...
            //功能说明】该方法用于裁剪画布，也就是设置画布的显示区域。在使用时，
            // 可以使用Rect对象来指定裁剪区，也可以通过指定矩形的4条边来指定裁剪区。
    ...
    public RectF getGridClippingRect() {
        ...
        /**将矩形插入（dx，dy）。 如果dx为正，则侧面为
          *向内移动，使矩形更窄。 如果dx是负数，那么
          *侧面向外移动，使矩形更宽。 同样如此
          *为dy和顶部和底部。**/
    ...
    protected float[] getTransformedPositions() {
        ...
        if (mGetTransformedPositionsBuffer.length != mYAxis.mEntryCount * 2) {
            //AxisRenderer.java   n = labelCount;   mAxis.mEntryCount = n;  labelCount = 6;
        ...
        for (int i = 0; i < positions.length; i += 2) {
            ...
            //AxisRenderer.java  mAxis.mEntries[i] = v;
