...
public class PieChart extends PieRadarChartBase<PieData> {
    ...
    private float[] mArcCenterAngles = new float[1];
...
//        canvas.drawColor(Color.WHITE);
        if (mData == null || mData.getDataSet() == null || mData.getDataSet().getEntryCount() == 0) {
            this.removeAllViews();
            // 画布颜色填充
    ...
    public void startDrawAnimation() {
        float totalAngle = getRotationAngle();
    ...
    private void calcArcCenterPoint() {
        float angleSum = 0;
        if (mArcCenterAngles.length != getDrawAngles().length) {
            mArcCenterAngles = new float[getDrawAngles().length];
        }
        for (int i = 0; i < getDrawAngles().length; i++) {
            mArcCenterAngles[i] = angleSum + getDrawAngles()[i] / 2;
            angleSum += getDrawAngles()[i];
        }
    }
    ...
    private void calcAngles() {
        ...
        //根据传入Data里 PieEntry集合大小创建初始环形区域角度数组长度
    ...
    public void setHighlight(int position) {
       mLegendRenderer.setCurrentPosition(position);
        if (position > mArcCenterAngles.length) {
            return;
        }
        float arcAngle = mArcCenterAngles[position];
        MPPointF center = getCenterCircleBox();
        float radius = getRadius() / 2;
        float arcCenterPointX = center.x + radius * (float) Math.cos((getRotationAngle() + arcAngle) * Utils.FDEG2RAD);
        float arcCenterPointY = center.y + radius * (float) Math.sin((getRotationAngle() + arcAngle) * Utils.FDEG2RAD);
        Highlight highlight = getHighlightByTouchPoint(arcCenterPointX, arcCenterPointY);
