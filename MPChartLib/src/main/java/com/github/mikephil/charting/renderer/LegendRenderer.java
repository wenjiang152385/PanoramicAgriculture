...
public class LegendRenderer extends Renderer {
        ...
        if (!mLegend.isLegendCustom()) {
            ...
            for (int i = 0; i < data.getDataSetCount(); i++) {
                ...
                if (dataSet instanceof IBarDataSet && ((IBarDataSet) dataSet).isStacked()) {
                    ...
                    if (pds.getLabel() != null && !pds.getLabel().equals("") && pds.getLabel() != "") {
                        // add the legend description label
                        computedEntries.add(new LegendEntry(
                                dataSet.getLabel(),
                                Legend.LegendForm.NONE,
    ...
    public LegendEntry[] setComputedEntries(int index) {
        LegendEntry[] entries = mLegend.getEntries();
        if (entries.length < 4) {
            return entries;
        } else {
            LegendEntry[] myEntries;
            if (index == 0) {
                myEntries = new LegendEntry[]{entries[entries.length - 1], entries[index], entries[index + 1]};
//                    Log.e("wjq","label = " + entries[entries.length - 1].toString());

                return myEntries;
            } else if (index == entries.length - 1) {
                myEntries = new LegendEntry[]{entries[index - 1], entries[index], entries[0]};
                return myEntries;
            } else {
                myEntries = new LegendEntry[]{entries[index - 1], entries[index], entries[index + 1]};
                return myEntries;
            }
        }
    ...
    public void renderLegend(Canvas c) {
        ...
        LegendEntry[] entries = setComputedEntries(mCurrentPosition);
